...
fov = 160.0
f = fisheye.focal_distance(fov=fov, image_dimension=image_dimension)
camera_f = 47.6083 * 2  # blender calculated value
...
retina_map_x, retina_map_y = fisheye.calculate_maps(
    points, image_dimension, fov=fov, camera_f=camera_f)
cv2.imwrite("map_on_distorted.png", fisheye.show_map(
    retina_map_x, retina_map_y, image_dimension, image_dimension))
...
for k in xrange(len(folders)):
    try:
        ...
        hsv = np.zeros_like(images[0][:, 0:images[0].shape[1] / 2])

        # this will be saved as input
        ...
        for s in xrange(len(sides)):
            ...
            ims = [fisheye_old.distort(i, dist_map_x, dist_map_y, camera_f)
                   for i in ims]
            ims = [cv2.GaussianBlur(i,(3,3),0) for i in ims]
            ...
            # try:
            allFlow = cv2.calcOpticalFlowFarneback(
                prev=ims[0], next=ims[1], flow=None,
                pyr_scale=0.5, levels=3, winsize=15,
                iterations=5, poly_n=3, poly_sigma=1.2, flags=0)
            ...
            # blacks for flow
            flow = [np.zeros((image_dimension, image_dimension)).astype(float)
                    for x in xrange(4)]
