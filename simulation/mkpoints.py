...
while 1:
    ...
    sys.stderr.write(str(dist / len(points)) + "\n")
    if dist / len(points) < 1e-5:
        break
...
"""
c = [middle(c) for c in cs]
x = np.array([f[0] for f in c])
y = np.array([f[1] for f in c])
z = np.array([f[2] for f in c])
fig = plt.figure()
ax = fig.add_subplot(111, projection='3d')
"""
